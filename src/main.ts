...
function getData() {
  const cleaned = carsData
    ...
    .filter(car => car.mpg != null && car.horsepower != null) as inputData;
 ...
 * MPG on the y-axis.
 */
function convertToTensor(data: inputData) {
...
async function trainModel(
  ...
  model.compile({
    optimizer: tf.train.adam(0.001), // Learning rate of 0.001
    loss: tf.losses.meanSquaredError,
    metrics: ["mse"],
  });

  const batchSize = 32;
  const epochs = 32; // Increased epochs for better convergence
